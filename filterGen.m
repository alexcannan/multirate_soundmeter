...
seconds = 0;
outputdata = analyzer(SOUND,hd,LPFnum);
...
output = sum(sum(outputdata,3),2);
%dB scale relative to unity
output = 10*log(output/1);

function meter = analyzer(ifs,bpf,lpf)
    fs = 25600;
    seconds = ceil(length(ifs)/fs);
    leftover = mod(length(ifs),fs);
    disp(strcat("Beginning analysis of ",num2str(seconds)," chunk input"));
    if ~(isvector(ifs) && (length(ifs) > fs))
        error('Input must be vector with length > 25600')
    end
    meter = zeros([seconds 7 3]);
    for i = 1:1:seconds %processing individual seconds
        disp(strcat("Analyzing chunk ",num2str(i),"/",num2str(seconds)));
        %fill temp chunk. Pad with zeros if not enough to fill.
        if (i == seconds) && (leftover ~= 0)
            tempifs = zeros([1 fs]);
            for k = 1:leftover
                tempifs(k) = ifs((seconds-1)*fs+k);
            end
        else
            tempifs = ifs((i-1)*fs+1:i*fs);
        end
